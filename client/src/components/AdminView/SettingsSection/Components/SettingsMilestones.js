...
class MilestoneRow extends React.Component {
  changeHandinLink = (event) => {
    ...
    post(`/api/milestones/${this.props.info._id}/update`, {
      handin_link: value,
    })
      .then((status) => {
        if (status === 204) {
  ...
  changeAutogradeStatus = (event) => {
    const target = event.target;
    const value = target.checked;
    post(`/api/milestones/${this.props.info._id}/set-autograde`, {
      autograde: value,
    })
      .then((status) => {
        if (status === 204) {
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{ display: "flex", width: "10vw" }}>
          <input
            ...
            value={info.deadline ? info.deadline.substring(0, 10) : ""}
          />
        </div>
        <div style={{ display: "flex", width: "10vw" }}>
          <input
            type="checkbox"
            onChange={this.changeSubmissionStatus}
            checked={!info.submission_closed}
          />
        </div>
        <div style={{ display: "flex", width: "10vw" }}>

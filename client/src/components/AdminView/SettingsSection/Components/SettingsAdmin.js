...
class SettingsAdmin extends React.Component {
  ...
  addAdmin = () => {
    post(`/api/class/${this.props.classId}/admins`, {
      admin_github_username: this.state.selected,
    })
      .then((status) => {
        if (status === 204) {
          this.setState({
  ...
  deleteAdmin = (github_username) => {
    if (confirm(`Are you sure you want to delete?`)) {
      delet(`/api/class/${this.props.classId}/admins`, {
        admin_github_username: github_username,
      })
        .then((status) => {
          if (status === 204) {
            this.getUsers();
